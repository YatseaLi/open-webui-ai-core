...
log.setLevel(SRC_LOG_LEVELS["OLLAMA-AI-CORE"])
...
def get_access_token():
    base_url = SAP_AI_API_URL+ "/v2/lm"
    ai_api_client = AIAPIV2Client(
    base_url= base_url,
    auth_url=SAP_AUTH_URL + "/oauth/token",
    client_id=SAP_CLIENT_ID,
    client_secret=SAP_CLIENT_SECRET,
    resource_group=SAP_RESOURCE_GROUP)
    token = ai_api_client.rest_client.get_token()

    log.info("Access Token of SAP AI Core retrieved")
    return token

def ai_core_authorize():
    app.state.TOKEN = get_access_token()
    app.state.HEADERS = {
            "Authorization": app.state.TOKEN,
            'ai-resource-group': SAP_RESOURCE_GROUP,
...
async def fetch_url(url):
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True, headers=app.state.HEADERS, connector=connector) as session:
    ...
    try:
        ...
        if r is not None:
            try:
                res = await r.json()
                if "error" in res:
                    error_detail = f"Ollama in SAP AI Core: {res['error']}"
            except:
                error_detail = f"Ollama in SAP AI Core: {e}"
...
async def get_ollama_versions(url_idx: Optional[int] = None):
    if app.state.config.ENABLE_OLLAMA_API:
        if url_idx == None:
            ...
            r = None
            try:
                r = requests.request(method="GET", url=f"{url}/api/version", headers=app.state.HEADERS, verify=True)
                r.raise_for_status()
...
async def download_file_stream(
    ...
    headers["Authorization"] = app.state.TOKEN
    headers["ai-resource-group"] = SAP_RESOURCE_GROUP
        ...
        async with session.get(file_url, headers=headers) as response:
            ...
            with open(file_path, "ab+") as file:
                ...
                if done:
                    ...
                    response = requests.post(url, headers=app.state.HEADERS, verify=True, data=file)
                    ...
                    if response.ok:
                        ...
                        raise "Ollama in SAP AI Core: Could not create blob, Please try again."
        ...
        try:
            with open(file_path, "rb") as f:
                ...
                if done:
                    ...
                    url = f"{ollama_url}/api/blobs/sha256:{hashed}"
                    response = requests.post(url, headers=app.state.HEADERS, verify=True, data=f)
                    ...
                    if response.ok:
                        ...
                        raise Exception(
                            "Ollama in SAP AI Core: Could not create blob, Please try again."
                        )
